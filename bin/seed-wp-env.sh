#!/usr/bin/env bash
#
# Seed WordPress with theme test fixtures. Runs inside the wp-env `cli`
# container via the lifecycleScripts.afterStart hook in .wp-env.json.
#
# Mirrors .ddev/orchestrate/project/20-seed-content.sh — keep the two in
# sync when adding fixtures.

set -euo pipefail

# Skip if already seeded (more than the default "Hello World" post).
...
wp plugin is-installed akismet && wp plugin uninstall akismet || true
wp plugin is-installed hello   && wp plugin uninstall hello   || true

# Activate the theme.
wp theme activate sovereignty

# Pretty permalinks so /<slug>/ URLs work without canonical redirects.
...
THEME_ASSETS="/var/www/html/wp-content/themes/sovereignty/assets"
...
# Taxonomy terms.
wp term create category "Technology"  --slug=technology  || true
wp term create category "Travel"      --slug=travel      || true
wp term create category "Photography" --slug=photography || true
