...
class Tags {
	...
	public static function posted_by(): void {
		\printf(
			...
			esc_attr( \sprintf( __( 'View all posts by %s', 'sovereignty' ), get_the_author() ) ),
	 ...
	 * @return void
	 */
	public static function posted_on( string $type = 'published' ): void {
	 ...
	 * @return void
	 */
	public static function post_id( ?string $post_id = null ): void {
		if ( $post_id !== null ) {
			echo 'id="' . esc_attr( $post_id ) . '"';
		} else {
			echo 'id="' . esc_attr( self::get_post_id() ) . '"';
		}
	}
	 ...
	 */
	public static function get_post_id(): string {
		 ...
		 */
		return apply_filters( 'sovereignty_post_id', $post_id, get_the_ID() ); // phpcs:ignore Apermo.WordPress.ImplicitPostFunction
	}
	...
	public static function the_content(): void {
		...
		if ( is_singular() ) {
			the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'sovereignty' ) );
			return;
		}

		$count = \str_word_count( wp_strip_all_tags( get_the_content() ) );

		if ( \defined( 'SOVEREIGNTY_EXCERPT' ) && \SOVEREIGNTY_EXCERPT && ( get_post_format() === false || $count > \SOVEREIGNTY_EXCERPT_COUNT ) ) { // @phpstan-ignore constant.notFound
			the_excerpt();
		} else {
			the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'sovereignty' ) );
		}
